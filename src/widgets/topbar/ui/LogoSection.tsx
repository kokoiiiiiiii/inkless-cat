...
const LogoSection = () => {
  const { t } = useI18n();
  const subtitle = t('topbar.subtitle');
  const logoAlt = t('topbar.logoAlt');
  const githubLabel = t('topbar.githubLabel');
  return (
    <div className="flex items-center gap-3">
      <img
        src={inklessCatLogo}
        alt={logoAlt === 'topbar.logoAlt' ? 'Inkless Cat logo' : logoAlt}
      ...
      <div className="flex flex-col">
        ...
        <span className="text-sm font-medium text-slate-500 dark:text-slate-400">
          {subtitle === 'topbar.subtitle' ? 'Inkless Cat Resume Editor' : subtitle}
        </span>
      </div>
      <a
        ...
        target="_blank"
        rel="noreferrer"
        aria-label={githubLabel === 'topbar.githubLabel' ? 'GitHub Repository' : githubLabel}
