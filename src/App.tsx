...
function App() {
  ...
  useEffect(() => {
    ...
    const targetScrollContainer =
      mobileView === 'editor' ? editorScrollContainerRef.current : previewScrollContainerRef.current;
    if (targetScrollContainer && typeof targetScrollContainer.scrollTo === 'function') {
      targetScrollContainer.scrollTo({ top: 0, behavior: 'smooth' });
    }
    if (isBrowser) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gradient-to-br from-slate-100 via-white to-slate-200 text-slate-900 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 dark:text-slate-100">
      ...
      <main className="mx-auto flex w-full max-w-[1400px] flex-1 flex-col gap-4 px-4 pb-16 pt-4 sm:gap-6 sm:pt-6 lg:grid lg:grid-cols-2 print:max-w-none print:px-0 print:pb-0 print:pt-0">
        ...
        <section
          className={`flex min-h-0 flex-col gap-4 rounded-3xl border border-slate-200/70 bg-white/85 p-4 shadow-soft backdrop-blur-xl dark:border-slate-800/70 dark:bg-slate-900/60 dark:shadow-[0_35px_60px_-25px_rgba(15,23,42,0.75)] print:hidden sm:p-6 ${
            showEditor ? '' : 'hidden'
